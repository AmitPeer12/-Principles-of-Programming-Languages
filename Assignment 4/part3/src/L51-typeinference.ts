...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    return bind(typeofExp(exp.val, tenv), (type: T.TExp) => bind(checkEqualType(type, exp.var.texp, exp), () => makeOk(T.makeVoidTExp())))
}
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    return bind(typeofExp(exp.val, tenv), (typeCurr: T.TExp) => bind(typeofExp(exp.var, tenv), (typeNew: T.TExp) =>
        bind(checkEqualType(typeCurr, typeNew, exp), () => makeOk(T.makeVoidTExp()))))
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const classTenv = E.makeExtendTEnv(R.map((v: A.VarDecl) => v.var, exp.varDecls), R.map((v: A.VarDecl) => v.texp, exp.varDecls), tenv);
    const methodsParamsTypes = R.map((b: A.Binding) => b.var.texp, exp.bindings);
    const methodsBodyTypes = mapResult((e: A.CExp) => typeofExp(e, classTenv), R.map((b: A.Binding) => b.val, exp.bindings));
    return bind(bind(methodsBodyTypes, (returnTypes: T.TExp[]) => checkEqualTypes(returnTypes, methodsParamsTypes, exp)),
        _ => makeOk(T.makeProcTExp(R.map((v: A.VarDecl) => v.texp, exp.varDecls), A.classExpToClassTExp(exp))));
};