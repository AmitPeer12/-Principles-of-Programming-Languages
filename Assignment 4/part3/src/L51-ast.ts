            ...
            isLetExp(e) ? [...e.body, ...map((b) => b.val, e.bindings)] :
                isLetrecExp(e) ? [...e.body, ...map((b) => b.val, e.bindings)] :
                    isAppExp(e) ? [e.rator, ...e.rands] :
                        isSetExp(e) ? [e.val] :
                            isDefineExp(e) ? [e.val] :
                                isClassExp(e) ? map((b) => b.val, e.bindings) : // L51
                                    isAtomicExp(e) ? [] :
                                        isLitExp(e) ? [] :
                                            e;
...
// L51
export interface ClassExp { tag: "ClassExp"; typeName: TVar, varDecls: VarDecl[]; bindings: Binding[]; }
export const makeClassExp = (typeName: TVar, varDecls: VarDecl[], bindings: Binding[]): ClassExp =>
    ({ tag: "ClassExp", typeName: typeName, varDecls: varDecls, bindings: bindings });
...
const parseClassExp = (params: Sexp[]): Result<ClassExp> =>
    params.length == 2 ? parseGoodClassExp(makeFreshTVar().var, params[0], params[1]) :
        (params.length != 4) || (params[0] != ':') ? makeFailure(`class must have shape (class [: <type>]? <varDecls> <bindings>) - got ${params.length} params instead`) :
...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {
    if (!isString(typeName)) return makeFailure('Invalid typeName');
    else if (isEmpty(varDecls) || !isArray(varDecls)) return makeFailure(`Invalid varDecls for classExp`);
    else if (!isGoodBindings(bindings)) return makeFailure('Malformed bindings in "class" expression');
    const vars = mapResult(parseVarDecl, map(b => b[0], bindings));
    const valsResult = mapResult(binding => parseL5CExp(binding[1]), bindings);
    return safe2((varDecls: VarDecl[], bindings: Binding[]) => makeOk(makeClassExp(makeTVar(typeName.toString()), varDecls, bindings)))
        (mapResult(parseVarDecl, varDecls), bind(valsResult, (vals: CExp[]) => bind(vars, (v: VarDecl[]) => makeOk(zipWith(makeBinding, v, vals)))));
};
...
const unparsevarDecls = (varDecls: VarDecl[], unparseWithTVars?: boolean): Result<string> =>
    bind(mapResult(field => unparseVarDecl(field, unparseWithTVars), varDecls),
        (varDecls: string[]) => makeOk(join(" ", varDecls)));

const unparseClassExp = (ce: ClassExp, unparseWithTVars?: boolean): Result<string> =>
    bind(unparsevarDecls(ce.varDecls),
        (varDecls: string) =>
            bind(unparseBindings(ce.bindings, unparseWithTVars),
                (bindings: string) =>
                    ce.typeName.var.startsWith("T") && !unparseWithTVars ? makeOk(`(class (${varDecls}) (${bindings}))`) :
                        makeOk(`(class : ${ce.typeName.var} (${varDecls}) (${bindings}))`)));
...
export const parsedToClassExps = (p: Parsed): ClassExp[] =>
    ...
    isProgram(p) ? p.exps.reduce((acc: ClassExp[], curr: Exp) => acc.concat(parsedToClassExps(curr)), []) :
        isProcExp(p) ? p.body.reduce((acc: ClassExp[], curr: Exp) => acc.concat(parsedToClassExps(curr)), []) :
            isIfExp(p) ? parsedToClassExps(p.then).concat(parsedToClassExps(p.alt)) :
                isLetExp(p) ? p.bindings.reduce((acc: ClassExp[], curr: Binding) => acc.concat(parsedToClassExps(curr.val)), []) :
                    isClassExp(p) ? [p] : [];

// L51 
export const classExpToClassTExp = (ce: ClassExp): ClassTExp =>
    makeClassTExp(ce.typeName.var, map((binding: Binding) => [binding.var.var, binding.var.texp], ce.bindings));
