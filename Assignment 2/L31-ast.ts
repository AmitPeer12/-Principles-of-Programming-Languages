...
export const parseClassExp = (fields: Sexp, methods: Sexp[]): Result<ClassExp> => {
    if (isEmpty(fields) || !isArray(fields) || !allT(isString, fields)) {
        return makeFailure(`Invalid fields for classExp`);
    } else if (!isGoodBindings(methods[0])) {
        return makeFailure('Malformed bindings in "class" expression');
    }
    const vars = map(b => b[0].toString(), methods[0]);
    const valsResult = mapResult(binding => parseL31CExp(binding[1]), methods[0]);
    return safe2((fields: VarDecl[], methods: Binding[]) => makeOk(makeClassExp(fields, methods)))
        (makeOk(map(makeVarDecl, fields)), bind(valsResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, vars, vals))));
};
...
const unparseClassExp = (ce: ClassExp): string =>
    `(class (${map((f: VarDecl) => f.var, ce.fields).join(" ")}) (${map((m: Binding) => `(${m.var.var} ${unparseL31(m.val)})`, ce.methods).join(" ")}))`
