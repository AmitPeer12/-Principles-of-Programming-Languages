...
const evalDefineExps = (def: DefineExp, exps: Exp[]): Result<Value> =>
    bind(bind(applicativeEval(def.val, theGlobalEnv), (v: Value) => makeOk(extendStore(theStore, v).vals.length - 1)), (addr: number) => {
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vals: Value[]) => {
        const addresses: number[] = map((val: Value) => extendStore(theStore, val).vals.length - 1, vals);
