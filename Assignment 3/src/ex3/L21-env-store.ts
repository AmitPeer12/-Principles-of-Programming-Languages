import { add, append, map, reduce, zipWith } from "ramda";
...
export const applyStore = (store: Store, address: number): Result<Value> => (store != null && store.vals.length > address) ?
    makeOk(unbox(store.vals[address])) : makeFailure("Invalid address");
...
const applyGlobalEnv = (env: GlobalEnv, v: string): Result<number> =>
    unbox(env.vars).includes(v) ? makeOk(unbox(env.addresses)[unbox(env.vars).indexOf(v)]) : makeFailure(`var not found ${v}`);
